...
window.FloSyncEngine = FloSyncEngine;
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  window.importFloFileFromOnboarding = function(fileInput) {
    ...
    reader.onload = function(e) {
      ...
      const parseRes = FloSyncEngine.parseFloFile(content, file.name);

      if (parseRes && parseRes.totalEntries > 0) {
        const res = FloSyncEngine.syncFloData(
          loggedDaysData,
          parseRes.latestPeriodDate || obSelectedLmpDateStr || userProfile.lmpFecha,
          parseRes.avgCycleLength || userProfile.duracionPromedioCiclo || 29,
          parseRes.avgPeriodLength || userProfile.duracionPromedioPeriodo || 5,
          parseRes.importedDays,
          parseRes.reconstructedCycles,
          parseRes.floPrediction
        );

        obHealthKitConnected = true;
        userProfile.floConectado = true;
        if (parseRes.latestPeriodDate) {
          obSelectedLmpDateStr = parseRes.latestPeriodDate;
          userProfile.lmpFecha = parseRes.latestPeriodDate;
        }
        if (parseRes.avgCycleLength) {
          userProfile.duracionCiclo = parseRes.avgCycleLength;
          userProfile.duracionPromedioCiclo = parseRes.avgCycleLength;
        }
        if (parseRes.avgPeriodLength) {
          userProfile.duracionPeriodo = parseRes.avgPeriodLength;
          userProfile.duracionPromedioPeriodo = parseRes.avgPeriodLength;
        }
        if (parseRes.floPrediction) {
          userProfile.floPrediction = parseRes.floPrediction;
        }
        userProfile.floRecordsCount = parseRes.totalEntries;
        userProfile.floCyclesCount = parseRes.reconstructedCycles.length;
        ...
        if (statusBadge) {
          statusBadge.textContent = '🟢 Flo Sincronizado (Datos Reales)';
          statusBadge.style.color = '#ff758f';
        }
        if (btnFloText) btnFloText.textContent = `✓ Archivo Flo Importado (${parseRes.reconstructedCycles.length} ciclos, ${parseRes.totalEntries} días)`;

        showInAppToast({
          title: 'Reporte de Flo Importado 🌸',
          message: `Se sincronizaron ${parseRes.reconstructedCycles.length} ciclos y ${parseRes.totalEntries} días reales de Flo para calibrar tu salud hormonal.`,
          icon: '🌸',
          badgeText: 'Flo Health Sync',
        ...
        showInAppToast({
          title: 'Archivo Flo no reconocido ⚠️',
          message: 'No se pudieron leer registros válidos en el archivo. Por favor verifica que sea una exportación de Flo en formato .json, .txt o .csv.',
          icon: '⚠️',
          badgeText: 'Error de importación',
          badgeIcon: 'warning',
          accentColor: '#f59e0b',
          duration: 5500
        });
  ...
  function getCyclePhaseForDate(dateObj) {
    ...
    const cycleLen = parseInt(userProfile?.duracionPromedioCiclo || userProfile?.duracionCiclo, 10) || 29;
    const periodLen = parseInt(userProfile?.duracionPromedioPeriodo || userProfile?.duracionPeriodo, 10) || 5;
    ...
    // Límites biológicos exactos calibrados con el modelo de Flo cuando esté sincronizado
    const floPred = userProfile?.floPrediction?.future || userProfile?.floPrediction?.current;
    const ovulationDay = (floPred && floPred.ovulation_day) ? floPred.ovulation_day : Math.max(periodLen + 2, cycleLen - 14);
    const ovulStart = (floPred && floPred.fertility_window_start) ? floPred.fertility_window_start : ovulationDay - 1;
    const ovulEnd = (floPred && floPred.fertility_window_end) ? floPred.fertility_window_end : ovulationDay + 1;
  ...
  async function renderSpotifyDashboardCard(animate = false, forceRotate = false) {
      ...
      cardContainer.innerHTML = `
        <div class="spotify-recommendation-card ${animClass}">
          ...
          <p class="spotify-card-desc">
            Conecta tu cuenta de Spotify para que <strong>${petName}</strong> elija la mejor canción según tu <strong>Fase ${displayPhase}</strong> y síntomas de hoy revisando tus artistas favoritos, álbumes y canciones guardadas.
          </p>
    ...
    try {
      ...
      const userLabel = userProf?.display_name || userProf?.email || 'Usuario';

      // CASO 1: Pista recomendada de su biblioteca real
        ...
        cardContainer.innerHTML = `
          <div class="spotify-recommendation-card spotify-connected ${animClass}">
            ...
            <div class="spotify-card-actions">
              ...
              <button class="btn-spotify-refresh" onclick="renderSpotifyDashboardCard(true, true)" title="Obtener otra recomendación de tu biblioteca para esta fase">
...
Genera para ella un reporte analítico de alto valor biológico respondiendo ÚNICAMENTE un objeto JSON válido con estas 5 claves (sin comillas triples, sin markdown, sin texto adicional):
{
    ...
    modal.innerHTML = `
      <div class="settings-modal-card animate-scale-up" style="max-height:85vh; overflow-y:auto;">
        ...
        <div class="settings-section-card" style="border-color: rgba(255, 77, 109, 0.4); background: rgba(255, 77, 109, 0.08);">
          ...
          <div style="font-size:0.75rem; color:#cbd5e1; margin-bottom:0.4rem;">
            Estado: <strong id="settings-flo-status">${(() => {
              if (!FloSyncEngine.isConnected()) return '<span style="color:#cbd5e1;">⚪ No conectado</span>';
              const meta = FloSyncEngine.getImportedMetadata();
              if (meta && (meta.historicalCyclesCount || meta.totalCycles)) {
                const cCount = meta.historicalCyclesCount || meta.totalCycles;
                const dCount = meta.recordsCount || meta.totalDays || 0;
                return `<span style="color:#ff758f;">🟢 Conectado con Datos Reales (${cCount} ciclos, ${dCount} días)</span>`;
              }
              return '<span style="color:#ff758f;">🟢 Conectado y Sincronizado con Flo</span>';
            })()}</strong>
          </div>
          <div style="display:flex; gap:0.45rem; flex-wrap:wrap;">
            ...
            <button class="settings-action-btn" style="background:rgba(255,255,255,0.08); border-color:rgba(255,255,255,0.2); color:#cbd5e1; padding:0.45rem 0.75rem;" onclick="document.getElementById('flo-file-input-settings').click()" title="Importar archivo exportado de Flo (.json, .txt o .csv)">
              <span class="material-symbols-outlined" style="font-size:0.95rem;">upload_file</span>
              <span>Subir Archivo (.json / .txt / .csv)</span>
            </button>
          </div>
        </div>
  ...
  window.importFloFileFromSettings = function(fileInput) {
    ...
    reader.onload = function(e) {
      ...
      if (parseRes && parseRes.totalEntries > 0) {
        ...
        if (statusEl) statusEl.innerHTML = `<span style="color:#ff758f;">🟢 Conectado con Datos Reales de Flo (${parseRes.reconstructedCycles.length} ciclos, ${parseRes.totalEntries} días)</span>`;

        showInAppToast({
          title: 'Reporte Flo Importado con Éxito 🌸',
          message: `Se calibraron ${parseRes.reconstructedCycles.length} ciclos reales y ${parseRes.totalEntries} días de síntomas históricos para máxima precisión en tus predicciones.`,
          icon: '🌸',
          badgeText: 'Flo Health Calibrado',
          badgeIcon: 'check_circle',
          accentColor: '#ff758f',
          duration: 5500
        });
        ...
        showInAppToast({
          title: 'Archivo Flo no reconocido ⚠️',
          message: 'No se pudieron procesar registros válidos en este archivo. Asegúrate de seleccionar el archivo .json, .txt (res.txt) o .csv exportado desde Flo.',
          icon: '⚠️',
          badgeText: 'Error de importación',
          badgeIcon: 'warning',
          accentColor: '#f59e0b',
  ...
  window.importFloFileFromModal = function(fileInput) {
    window.closeFloSyncModal();
    window.importFloFileFromSettings(fileInput);
  };
