...
class FloSyncEngine {
  ...
  static normalizeDateStr(rawDate) {
    ...
    // Formatos DD/MM/YYYY o MM/DD/YYYY
    ...
    if (slashMatch) {
      ...
      if (p1 > 12) {
        ...
        // Estándar latinoamericano / Flo export: DD/MM/YYYY
        day = p1;
        month = p2;
      }
  ...
  static extractCyclesFromPeriodDays(periodDates, cycleLenFallback = 28, periodLenFallback = 5) {
    ...
    // Agrupar días de sangrado en episodios menstruales (gap > 4 días separa episodios)
    const episodes = [];
    ...
    for (let i = 0; i < episodes.length - 1; i++) {
      ...
      // Rango fisiológico admisible (18 a 60 días)
  ...
  static parseFloJSON(jsonText) {
    ...
    try {
      ...
      // 1. Ciclos explícitos en el archivo
      ...
      // 2. Extraer arreglo de días de los diversos esquemas de Flo
      let daysArray = [];
    ...
    reader.onload = function(e) {
      ...
      if (parseRes && parseRes.totalEntries > 0) {
        const res = FloSyncEngine.syncFloData(
          ...
          parseRes.avgCycleLength || userProfile.duracionPromedioCiclo || 28,
        ...
        if (btnFloText) btnFloText.textContent = `✓ Archivo Flo Importado (${parseRes.totalEntries} días reales)`;
        ...
        showInAppToast({
          ...
          message: 'No se pudieron leer registros válidos en el archivo. Por favor verifica que sea una exportación de Flo en formato .json o .csv.',
          icon: '⚠️',
  ...
  window.importFloFileFromSettings = function(fileInput) {
    ...
    reader.onload = function(e) {
      ...
      if (parseRes && parseRes.totalEntries > 0) {
        ...
        if (statusEl) statusEl.innerHTML = `<span style="color:#ff758f;">🟢 Conectado con Datos Reales de Flo (${parseRes.totalEntries} días)</span>`;
        ...
        showInAppToast({
          title: 'Archivo Flo no reconocido ⚠️',
          message: 'No se pudieron procesar registros válidos en este archivo. Asegúrate de seleccionar el archivo .json o .csv exportado desde Flo.',
          icon: '⚠️',
          badgeText: 'Error de importación',
          badgeIcon: 'warning',
          accentColor: '#f59e0b',
